...
# Drop NaNs as during training
df.dropna(inplace=True)
print(f"\n⚠️ Dropping rows containing NaN values, new shape: {df.shape}")
...
X = df.drop(columns=['id', 'season', 'date', 'result', 'dl_applied', 'winner', 'player_of_match', 'umpire1', 'umpire2', 'umpire3'])
y = df['winner']

# Check for non-numeric features
non_numeric_cols = X.select_dtypes(exclude=[np.number]).columns.tolist()
if non_numeric_cols:
    print(f"\n❌ Found non-numeric feature columns causing errors: {non_numeric_cols}")
    print("Exiting: please check your feature engineering.")
    exit(1)
...
print(f"\n✅ Saved {len(X.columns)} feature columns to {FEATURE_COLUMNS_PATH}")
...
print(f"✅ Saved trained stacked model to {MODEL_PATH}")
